...
Task1–4 first-frame CSV initialization records and their XML snapshots are included under `mujoco_recordings`; see its README and manifest. Full CSV trajectories, videos, HSSD dataset, policy weights and Python environments are not included. Supply those separately. Keep this checkout at `HUMANOID_VLA_MJ_ROOT`; install compatible Isaac Sim separately.
