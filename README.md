...
Task1–6 first-frame CSV initialization records and their XML snapshots are included under `mujoco_recordings`; see its README and manifest. The four Task1–6 HSSD backgrounds and their dependencies are published in `ql3c20/SIMPLE_eval_env:icra2027` under `data/scenes/hssd`. Full CSV trajectories, videos, policy weights and Python environments are not included. Supply those separately. Keep this checkout at `HUMANOID_VLA_MJ_ROOT`; install compatible Isaac Sim separately.
